...
class HrPayslip(models.Model):
    ...
    def get_worked_day_lines(self, contracts, date_from, date_to):
        ...
        @return: retorna una lista de dicts con los días trabajados y otros cálculos
        """
        res = []
        for contract in contracts.filtered(lambda c: c.resource_calendar_id):
            ...
            rest_days = self._compute_rest_days(contract, day_from, day_to) 
            ...
            holiday_days = self._compute_holiday_days(contract, day_from, day_to)
    ...
    def _compute_rest_days(self, contract, day_from, day_to):
        """
        Cálculo de los días de descanso (sábados y domingos) dentro del período de la nómina.
        """
        rest_days = 0.0
    ...
    def _compute_night_bonus(self, contract, day_from, day_to):
        """
        Cálculo del Bono Nocturno basado en los días realmente trabajados en el turno nocturno.
        """
        employee = contract.employee_id
        night_bonus_days = 0.0
        night_bonus_hours = 0.0

        work_intervals = employee.list_leaves(day_from, day_to, calendar=contract.resource_calendar_id)

        for day, hours, leave in work_intervals:
            if hours > 0:
                start_hour = day.hour
                end_hour = (day + relativedelta(hours=hours)).hour

                if start_hour >= 19 or end_hour <= 7:
                    night_bonus_days += 1
                    night_bonus_hours += hours

        return {
            "name": _("Bono Nocturno"),
            "sequence": 3,
            "code": "NIGHT0",
            "number_of_days": night_bonus_days,
            "number_of_hours": night_bonus_hours,
            "contract_id": contract.id,
        }
    ...
    def _compute_special_days(self, contract, day_from, day_to):
        """
        Special Days computation (with 0 as default).
        @return: returns a dictionary with special days for the period of the payslip.
        """
        special_days = 0.0
        return {
            ...
            "sequence": 4,
            "code": "SPECIAL0",
            "number_of_days": special_days,
            "number_of_hours": special_days,
            "contract_id": contract.id,
        }

    def _compute_holiday_days(self, contract, day_from, day_to):
        """
        ...
        return {
            ...
            "number_of_hours": holiday_days,
            "contract_id": contract.id,
        }

    def _compute_overtime(self, contract, day_from, day_to):
        """
        Overtime computation (with 0 as default).
        @return: returns a dictionary with overtime+9 days for the period of the payslip.
        """
        overtime = 0.0
        return {
            "name": _("Horas Extra"),
    ...
    def _compute_mondays(self, contract, day_from, day_to):
        """
        Cálculo de la cantidad de lunes dentro del período de la nómina.
        """
        monday_count = 0
        current_date = day_from
